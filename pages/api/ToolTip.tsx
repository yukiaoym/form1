...
export default function ToolTip({label}:LabelProps) {
    ...
    const ToolTipDict: {[index: string]: string} = {
        ...
        "CM_希望アクセスURL_1": "本サービスにアクセスするURLに設定されます。英数半角小文字とハイフンのみ利用可。\n例）https://***.cybermail.jpの「***」を指します。",
        "ユーザー認証方式": "CYBERMAILΣへログインする際の認証方式となります。\nデフォルト設定はCYBERMAILとなります。\nLDAP認証またはSAML認証をご利用の場合は、ユーザ認証方式を変更してください。",
        "SAML認証_担当者名": "SAML認証の技術的なご担当者様の氏名をご記入下さい。",
        "SAML認証_担当者メールアドレス": "SAML認証の技術的なご担当者様のメールアドレスをご記入下さい。",
        "SAML認証_IdPサービス名": "ご利用のIdPサービスを選択して下さい。",
        "SAML認証_IdPメタデータ": "ご利用のIdPサービスのメタデータを添付して下さい。",
        "LDAP認証_サーバー": "お客様環境のLDAP/Active DirectoryサーバーのグローバルIPアドレス",
        "LDAP認証_ポート番号": "お客様環境のLDAP/Active Directoryサーバーのポート番号",
        "LDAP認証_BaseDN": "お客様環境のLDAP/Active Directoryサーバーの検索起点となるDN",
        "LDAP認証_文字コード": "お客様環境のLDAP/Active Directoryサーバーの文字コード",
        "LDAP認証_ログインアカウント": "お客様環境のLDAP/Active Directoryサーバーの検索権限を持っているアカウント",
        "LDAP認証_ログインパスワード": "お客様環境のLDAP/Active Directoryサーバーの検索権限を持っているアカウントのパスワード",
        "LDAP認証_フィルタ": "お客様環境のLDAP/Active Directoryサーバーの検索で使用するフィルタ",
        "オプション_WebAPI": "CYBERMAIL ΣのWebAPIをご利用の場合は「利用する」を選択してください。",
        "WebAPI_アクセス元IPアドレス": "WebAPIを利用する場合は、送信元IPアドレスをご記入ください。",
        "MailGates_IPアクセス制御": "情報漏えい対策・アンチウイルス・アンチスパム管理画面へのアクセス制御をご利用の場合はご記入ください。\n制御が不要な場合は空欄にしてください。\n※全ユーザが対象となります。",
        "OP_暗号化ファイルスキャン機能": "パスワード付きZIPファイルのウイルススキャン機能",
        "OP_DKIM": "DKIM認証とは、送信者のなりすましやメールの改ざんを防ぐための仕組みです。\n送信メールにDKIMヘッダを付与する場合は「利用する」を選択して下さい。\n環境提供時にお客様ドメインのDKIMレコードへの登録していただく公開鍵のデータを提供いたします。",
